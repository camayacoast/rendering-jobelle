...
<body>
    ...
    <div class="wrapper">
        ...
        <h3 style="text-transform: uppercase; text-align: center;">BOOKING STATUS: <span
                style="color: orange">{{ $booking['status'] }}</span></h3>
        ...
        <table border="0" width="100%" style="margin-top: 32px; margin-bottom: 32px;">
            ...
            <tr>
                <td>Customer</td>
                <td>{{ $booking['customer']['first_name'] }} {{ $booking['customer']['last_name'] }}
                    ({{ $booking['customer']['user_type'] }})</td>
            </tr>
            @if (isset($booking['bookedBy']))
                <tr>
                    <td>Booked by</td>
                    <td>{{ $booking['bookedBy']['first_name'] }} {{ $booking['bookedBy']['last_name'] }}
                        ({{ $booking['bookedBy']['user_type'] }})</td>
                </tr>
            @endif
            <tr>
                <td>Date of visit</td>
        ...
        <table border="0" cellpadding="8" width="100%" style="margin-top: 0px; margin-bottom: 0px;">
            ...
            @foreach ($booking['guests'] as $guest)
                <tr>
                    <td align="left" style="text-transform: uppercase;">{{ $guest['first_name'] }}
                        {{ $guest['last_name'] }}</td>
                    <td align="left" style="text-transform: capitalize;">
                        {{ implode(', ',collect($guest['guestTags'])->pluck('name')->all()) }}</td>
        ...
        @if (collect($booking['invoices'])->sum('balance') > 0)
            <div class="page-break-before"></div>
            @include('pdf.booking.components.payment_instructions', [
                'booking' => $booking,
                'camaya_transportations' => $camaya_transportations,
            ])
        @endif
