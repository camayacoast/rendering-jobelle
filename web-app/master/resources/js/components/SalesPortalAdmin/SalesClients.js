...
import { EllipsisOutlined, UserAddOutlined, EditOutlined, ImportOutlined, PrinterOutlined, DeleteOutlined} from '@ant-design/icons';
...
export default function Page(props) {
    ...
    const [deleteClientQuery, { isLoading: deleteClientQueryLoading}] = SalesAdminPortalService.deleteClient();
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                return (
                    <>
                       <Link style={{marginRight: '5px'}} to={`view-client/${record.id}`}><Button icon={<EditOutlined/>} /></Link>
                       <Button type="danger" size="small" onClick={() => handleDelete(record.id)}>Cancel <DeleteOutlined/></Button>
                    </>
                );
            }
        }
   ...
   const handleDelete = id => {
      ...
      if(!confirm('Are you sure you want to cancel this client?')) return false;

      if(deleteClientQueryLoading) return false;

      deleteClientQuery({id: id}, {
        onSuccess: (res) => {

            message.success('Client Successfuly Deleted')

            salesClientsListQuery.refetch();
        },
        onError: (e) => {
