...
export default function Page(props) {
    ...
    const mealDataQuery = DashboardService.dttMeal(startDate, endDate);
    const [mealData, setMealData] = useState([]);


    const mealExportData = DashboardService.dttMealExport(startDate, endDate);
    const [mealExport, setExportMeal] = useState([]);


    useEffect(() => {
    if (mealDataQuery.isSuccess) {
        setExportMeal(mealExportData.data);
    }
}, [mealExportData]);
    ...
    useEffect(() => {
        if (mealDataQuery.isSuccess) {
            mealExportData;
            setMealData(mealDataQuery.data);
        }
    }, [mealDataQuery]);

      // Calculate the total remaining value by summing all meal types
    const totalRemaining = mealData.reduce((total, mealItem) => {
        return total + parseInt(mealItem.total_remaining, 10);
    }, 0);

    React.useEffect(()=> {
        // console.log(dashboardDataQuery.data);
        dashboardDataQuery.refetch();
        mealDataQuery.refetch();
        mealExportData.refetch();
    },[startDate, endDate]);
            ...
            <Row gutter={[48,48]} className="mt-4">
                ...
                <Col xl={24}>
                    ...
                    >
                        ...
                        <Col xl={6}>
                                <Card>
                                    <Statistic title="Total" value={totalRemaining} />
                                    <ExcelFile filename={`Meal_Export_${moment().format('YYYY-MM-DD HH:mm:ss')}`} element={<Button style={{ float: 'right' }} size="small"><PrinterOutlined /></Button>}>
                                    <ExcelSheet data={mealExport} name="meal_export">
                                        <ExcelColumn label="Guest Meal ID" value="guest_meal_id" />
                                        <ExcelColumn label="Booking Reference" value="booking_reference" />
                                        <ExcelColumn label="Guest Reference Number" value="guest_reference_number" />
                                        <ExcelColumn label="Guest's Name" value="guests_name" />
                                        <ExcelColumn label="Age" value="age" />
                                        <ExcelColumn label="Meal Name" value="meal_name" />
                                        <ExcelColumn label="Meal Type" value="meal_type" />
                                        <ExcelColumn label="Remaining" value="remaining" />
                                        <ExcelColumn label="Booking Type" value={col => col.booking_type === 'DT' ? 'Day tour' : 'Overnight'} />
                            ...
                            {mealData.map((mealItem, index) => (
                                <Col xl={6} key={index}>
                                    <Card>
                                        <Statistic title={mealItem.meal_type} value={mealItem.total_remaining} />
                                    </Card>
                                </Col>
