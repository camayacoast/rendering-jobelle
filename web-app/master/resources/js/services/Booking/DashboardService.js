...
const dttMeal = (startDate, endDate) => {

    if (startDate && endDate) {
        return useQuery("dtt-meal-data", async () => {
            const { data } = await Http.get(`/api/booking/guest/meals-report?start_date=${moment(startDate).format('YYYY-MM-DD')}&end_date=${moment(endDate).format('YYYY-MM-DD')}`);
            return data;
        });
    } else {
        return useQuery("dtt-meal-data", async () => {
...
const dttMealExport = (startDate, endDate) => {

    if (startDate && endDate) {
        return useQuery("dtt-meal-export", async () => {
            const { data } = await Http.get(`/api/booking/guest/meals-export?start_date=${moment(startDate).format('YYYY-MM-DD')}&end_date=${moment(endDate).format('YYYY-MM-DD')}`);
            return data;
        });
    } else {
        return useQuery("dtt-meal-export", async () => {
