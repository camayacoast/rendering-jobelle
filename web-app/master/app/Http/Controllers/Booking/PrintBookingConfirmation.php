...
class PrintBookingConfirmation extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $booking = Booking::where('reference_number', $request->booking_reference_number)
                ...
                $uniqueMeals = [];
        if (!empty($request->booking_reference_number) || !is_null($request->booking_reference_number)) {
            $guest_meals = Guest::select('meals.type','meals.name')
            ->where('guests.booking_reference_number', $request->booking_reference_number)
            ...
            if ($booking->status == 'confirmed') {
                ...
                return \PDF::loadView('pdf.booking.booking_confirmation', ['booking' => $booking, 'camaya_transportations' => $camaya_transportations,'guest_meals'=>$uniqueMeals])->stream();
            } else if ($booking->status == 'pending') {

                return \PDF::loadView('pdf.booking.booking_pending', ['booking' => $booking, 'camaya_transportations' => $camaya_transportations,'guest_meals'=>$uniqueMeals])->stream();
            }
    }
}
