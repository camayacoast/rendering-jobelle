...
class PackageList extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $packages = Package::with([
            ...
            // ->where('mode_of_transportation','=','camaya_transportation_bus')
            ->orderBy('name')
            ->select('packages.*')
            ->groupBy('packages.id')
            ->get();
        ...
        if (isset($request->start_date)) {
            ...
            foreach ($packages as $package) {
                ...
                if ($package['mode_of_transportation'] === 'camaya_transportation_bus' || $package['mode_of_transportation'] === 'camaya_transportation' || $package['mode_of_transportation'] === 'camaya_transportation_ferry') {

                    $arrival_schedules = "";
                    if ($package['mode_of_transportation'] === 'camaya_transportation_bus') {

                        $arrival_schedules = $this->trip_schedule('bus', $role_ids, $package, $override_roles, $request);
                    }else if($package['mode_of_transportation'] === 'camaya_transportation' || $package['mode_of_transportation'] === 'camaya_transportation_ferry'){
                    // if($package['mode_of_transportation'] == "camaya_transportation" || $package['mode_of_transportation'] == "camaya_transportation_ferry") {
                        $arrival_schedules = $this->trip_schedule('ferry', $role_ids, $package, $override_roles, $request);
                    }else{
                        $arrival_schedules = [];
                    }
                    ...
                    if (count($arrival_schedules)) {
                        ...
                        if ($package['mode_of_transportation'] === 'camaya_transportation_bus') {
                            ...
                            $departure_schedules = $this->departure_schedules('bus',$arrival_schedules, $role_ids, $package, $override_roles, $request);
                        } else{
                        // if($package['mode_of_transportation'] == "camaya_transportation" || $package['mode_of_transportation'] == "camaya_transportation_ferry") {
                            $departure_schedules = $this->departure_schedules("ferry",$arrival_schedules, $role_ids, $package, $override_roles, $request);
                        }
                ...
                $packages_with_availability[] = [
                    ...
                    'id' => $package['id'],
                    'meal_inclusion'=>$package['meal_inclusion'],
    ...
    protected function trip_schedule($transportation_type, $role_ids, $package, $override_roles, $request)
    {
        return \App\Models\Transportation\SeatSegment::where(function ($q) use ($role_ids, $request, $package, $override_roles) {
            if (!$request->user()->hasRole($override_roles)) {
            ...
            ->whereIn('origin.code', ['EST','MTP',"FBT","FCT"]) // Set origin as EST
            ...
            ->where("transportations.type", '=', $transportation_type)
            ->get();
    }

    protected function departure_schedules($transportation_type,$arrival_schedules, $role_ids, $package, $override_roles, $request){
        return \App\Models\Transportation\SeatSegment::where(function ($q) use ($role_ids, $request, $package, $override_roles) {
            if (!$request->user()->hasRole($override_roles)) {
                $q->whereRaw('seat_segments.id in (select seat_segment_id from seat_segment_allows where user_id = ?)', [$request->user()->id]);
                $q->orWhereRaw('seat_segments.id in (select seat_segment_id from seat_segment_allows where role_id in (?))', [$role_ids]);
            ...
            ->join('locations as origin', 'routes.origin_id', '=', 'origin.id')
            ->join('locations as destination', 'routes.destination_id', '=', 'destination.id')
            ->whereIn('destination.code', ["EST","MTP","FBT","FCT"]) // Set origin as CMY
