...
class CreateBookingAsGuest extends Controller
{
    ...
    public function __invoke(CreateBookingAsGuestRequest $request)
    {
        ...
        // }
        // seperate ferry and bus on saving
        ...
        } else if($selectedBookingPackages[0]->mode_of_transportation == "camaya_transportation" || $selectedBookingPackages[0]->mode_of_transportation == "camaya_transportation_ferry" ) {
        ...
        $newBooking = Booking::create([
            ...
            'mode_of_transportation' => isset($request->guest_vehicles) ? 'own_vehicle' : ($modeOfTransportation ?? 'undecided'),
            'eta' => $request->eta,
            'portal' => 'website',
        ...
        if (!empty($meals) && count($meals) > 0) {
            $totalPax = $adult_count + $kid_count;
            $overallTotal = $totalPax / $totalPax;
            $guest_meals = [];
