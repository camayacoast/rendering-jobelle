...
class Create extends Controller
{
    ...
    public function __invoke(CreateRequest $request)
    {
        ...
        // return $request->all();
        // return $request->user()->roles[0]['name'];
        ...
        $daily_limit = Setting::where('code', 'ADMIN_DAILY_LIMIT')->first()->value ?? 1400;
        ...
        $check_daily_limit_per_day = DailyGuestLimit::whereIn('date', [$arrival_date->format('Y-m-d')])->where('category', 'Admin')->get();
        ...
        $total_pax = $request->adult_pax + $request->kid_pax;
        ...
        if ($request->firstTrip) {
            ...
            if ($request->infant_pax > 0 || $request->kid_pax > 0 || $request->adult_pax > 0) {
                ...
                if ($request->infant_pax > $available_slot_for_infant) {
                    $connection->rollBack();
                    // $booking_error_count++;
                ...
                if ($request->kid_pax > $available_slot_for_kid) {
                    $connection->rollBack();
                    // $booking_error_count++;
                ...
                if ($request->adult_pax > $available_slot_for_adult) {
                    $connection->rollBack();
                    // $booking_error_count++;
        ...
        $customer_user_details = Customer::where('id', $request->customer)->with('user')->first();

        // Land Allocation Checking
        if ($request->mode_of_transportation != 'camaya_transportation' && $request->sd_tag) {
            $land_allocations = LandAllocation::whereIn('date', $booking_dates)->where('owner_id', $request->sd_tag)->where('entity', 'RE')->where('status', 'approved')->get();
            ...
            foreach ($land_allocations as $land_allocation) {
                $available = ($land_allocation['allocation'] - $land_allocation['used']) - $total_pax;
                ...
                LandAllocation::where('id', $land_allocation['id'])->increment('used', $total_pax);
            }
        }
        $selectedBookingProducts = Product::whereIn('id', collect($request->selectedProducts)->pluck('product_id')->all())->with('productPass')->get();
        ...
        $selectedBookingPackages = Package::whereIn('id', collect($request->selectedPackages)->pluck('package_id')->all())
            ->with('packageInclusions.product')
            ->with('packageRoomTypeInclusions.room_type')->get();

        /**
         * Create booking record
         */
        $modeOfTransportation = $request->mode_of_transportation;
        //  if($request->mode_of_transportation === "camaya_vehicle"){
        //     // $camaya_transportation['transportation']['type'] === 'ferry'
        //     $modeOfTransportation = "camaya_transportation_bus";
        //  }else{
        //     $modeOfTransportation = $request->mode_of_transportation;
        //  }

        if($request->mode_of_transportation === "camaya_transportation"){
            if(count($request->firstTrip) > 0){
                if( $request->firstTrip["transportation"]["type"] === "bus"){
                    $modeOfTransportation = "camaya_transportation_bus";
                }else{
                ...
                if( $request->secondTrip["transportation"]["type"] === "bus"){
                    $modeOfTransportation = "camaya_transportation_bus";
                }else{
                    $modeOfTransportation = "camaya_transportation";
                }
        ...
        $newBooking = Booking::create([
            'user_id' => $customer_user_details->user->id ?? null,
            ...
            'agent_id' => $request->agent_tag,
            'sales_director_id' => $request->sd_tag,
        ]);
        ...
        foreach ($allGuests as $guest) {
            ...
            if ($request->isGolf) {
                // $guest_tee_time_schedule_ids = $guest->guest_tee_time;

                $tee_time_schedules = TeeTimeSchedule::whereIn('id', $guest['guest_tee_time'])
        ...
        $guest_id = [];
        if ($request->breakfast !== 0 && $request->breakfast !== null && $request->breakfast !== '') {
            $meals->push($request->breakfast);
        }
        if ($request->lunch !== 0 && $request->lunch !== null && $request->lunch !== '') {
            $meals->push($request->lunch);
        }
        if ($request->dinner !== 0 && $request->dinner !== null && $request->dinner !== '') {
            $meals->push($request->dinner);
        }



        if (!empty($meals) && count($meals) > 0) {
            $overallTotal = $total_pax / $total_pax;
            $guest_meals = [];
            foreach($guests as $guest){
                ...
                foreach ($meals as $meal) {
                    $createdMeal[] = [
                        "meal_id" => $meal,
                        "guest_id" => $guest->id,
                        "remaining" => $overallTotal,
                        "status" => 0
                    ];
                }
            }
             GuestMeals::insert($createdMeal);
        }
        ...
        // if ($selectedBookingPackages[0]->mode_of_transportation == "camaya_transportation_bus") {
        //     $modeOfTransportation = "camaya_transportation_bus";
        // } else {
        //     $modeOfTransportation = "camaya_transportation";
        // }
        ...
        foreach ($newBooking->guests as $guest) {
                ...
                if ($nights >= 1) {
                    for ($i = 0; $i < $nights; $i++) {
                        ...
                        if ($request->bookingAsWalkIn) {
                            ...
                            $price = ($item['walkin_price'] == null) ? $per_guest_package_selling_price : $item['walkin_price'];
                        }
            ...
            if ($request->firstTrip) {
                ...
                // Increment invoice total cost
                $ticket_original_price = ($guest['type'] == 'infant') ? 0 : $seat_segment_1->rate; // Change this to actual ticket price
                ...
                $ticket_selling_price = $guest['type'] == 'infant' ? 0 : $ticket_selling_price;
            ...
            if ($request->secondTrip) {
                ...
                if ($available_seat_2 <= 0) {
                    $connection->rollBack();
                    $booking_error_count++;
                    return response()->json(['error' => 'SECOND_TRIP_FULLY_BOOKED', 'message' => 'Second trip is fully booked.'], 400);
                }
                ...
                // Create inclusion for camaya transportation ticket
                // Increment invoice total cost
                $ticket_original_price = ($guest['type'] == 'infant') ? 0 : $seat_segment_2->rate; // Change this to actual ticket price
                $ticket_selling_price = collect($request->selectedPackages)->firstWhere('camaya_transportation_available', true) ? 0 : $ticket_original_price;
                $ticket_selling_price = $guest['type'] == 'infant' ? 0 : $ticket_selling_price;
                $invoice_total_cost = $invoice_total_cost + $ticket_selling_price;
        ...
        foreach ($perBookingProducts->all() as $item) {
            ...
            $selectedProduct = collect($request->selectedProducts)->firstWhere('product_id', $item['id']);
            ...
            if (!$selectedProduct['quantity']) {
                ...
                $invoice_total_cost = $invoice_total_cost + $price * $selectedProduct['quantity'];
            }
        ...
        foreach ($perBookingPackages->all() as $item) {
            // Increment invoice total cost
            $selectedPackage = collect($request->selectedPackages)->firstWhere('package_id', $item['id']);

            // $price = $item['selling_price'];

            // if ($request->bookingAsWalkIn) {
            //     $price = ($item['walkin_price'] == null) ? $item['selling_price'] : $item['walkin_price'];
            // }
            ...
            if ($nights >= 1) {
                for ($i = 0; $i < $nights; $i++) {
                    ...
                    if ($request->bookingAsWalkIn) {
                        $selling_price_type = 'Walkin Rate';
                        $price = ($item['walkin_price'] == null) ? $per_booking_package_selling_price : $item['walkin_price'];
                    }
                    ...
                    if (!$selectedPackage['quantity']) {
                        ...
                        $invoice_total_cost = $invoice_total_cost + $price * $selectedPackage['quantity'];
                    }
                ...
                if ($request->bookingAsWalkIn) {
                    $selling_price_type = 'Walkin Rate';
                    $price = ($item['walkin_price'] == null) ? $per_booking_package_dtt_price : $item['walkin_price'];
                }
                            ...
                            foreach ($period as $date_period) {
                                if ($date_period->format('Y-m-d') != $departure_date->format('Y-m-d')) {
                                    ...
                                    if ($selling_rate == $last_room_rate) {
                                        // $rooms_to_save_as_inclusions[$batch][][$last_room_rate][] = $date_period->format('Y-m-d');
                                        $rooms_to_save_as_inclusions[$batch]['rate'] = $last_room_rate;
                                        // $rooms_to_save_as_inclusions[$batch]['rack_rate'] = $room_type->rack_rate;
                                        $rooms_to_save_as_inclusions[$batch]['dates'][] = $date_period->format('Y-m-d');
                                    } else {
                                        $batch = $batch + 1;
                                        $rooms_to_save_as_inclusions[$batch]['rate'] = $selling_rate;
        ...
        if ($request->selectedRoomTypes) {
            // return $request->selectedRoomTypes;
            // Check if the rooms are still available
            // $connection->rollBack();
            ...
            foreach ($request->selectedRoomTypes as $room_types_to_reserve) {
                ...
                if ($room_types_to_reserve['quantity']) {
                    ...
                    if (!isset($roomAvailability) || (count($roomAvailability['available_room_ids']) < $room_types_to_reserve['quantity'])) {
                        $connection->rollBack();
                        $booking_error_count++;
                        return response()->json(['error' => 'ROOM_FULLY_BOOKED_ALA_CARTE_1', 'message' => 'Fully booked for room type: (' . $room_type->property->name . ') ' . $room_type->name], 400);
                    }
                        ...
                        foreach ($period as $date_period) {
                            if ($date_period->format('Y-m-d') != $departure_date->format('Y-m-d')) {
                                ...
                                if ((($room_allocation['allocation'] - $room_allocation['used']) - 1) < 0) {
                                    $connection->rollBack();
                                    $booking_error_count++;
                                    return response()->json(['error' => 'ROOM_FULLY_BOOKED_ALA_CARTE_2', 'message' => 'Fully booked for room type: (' . $room_type->property->name . ') ' . $room_type->name], 400);
                                    // return 'Fully booked for room type: ('. $room_type->property->name .') '.$room_type->name;
                                }
        ...
        if ($request->saveSend === true) {
            ...
            Mail::to($customer_user_details->email)
                ->cc($additional_emails)
                ->send(new NewBooking($booking, $camaya_transportations,$guest_id));
        }


        return response()->json($newBooking, 200);
    }
}
