...
class WebsitePackageList extends Controller
{
        ...
        if (isset($request->start_date)) {
            ...
            foreach ($packages as $package) {
                    ...
                    if ($package['mode_of_transportation'] === 'camaya_transportation_bus' || $package['mode_of_transportation'] === 'camaya_transportation' || $package['mode_of_transportation'] === 'camaya_transportation_ferry') {
                        ...
                        // return $package['mode_of_transportation'];
                        /**
                         * Start
                         * s
                         */
                        ...
                        if($package['mode_of_transportation'] === 'camaya_transportation_bus'){
                            $arrival_schedules = $this->arrival_schedule('bus',$customer_role,$package,$request);
                        }else if($package['mode_of_transportation'] === 'camaya_transportation' || $package['mode_of_transportation'] === 'camaya_transportation_ferry'){
                            $arrival_schedules = $this->arrival_schedule('ferry',$customer_role,$package,$request);
                        }else{
                            $arrival_schedules = [];
                        }
                        ...
                        if (count($arrival_schedules)) {
                            if($package['mode_of_transportation'] === 'camaya_transportation_bus'){
                                $departure_schedules = $this->departure_schedule("bus",$arrival_schedules,$customer_role,$package,$request);
                            }else{
                                $departure_schedules =  $this->departure_schedule("ferry",$arrival_schedules,$customer_role,$package,$request);
                            }
    ...
    protected function arrival_schedule($transportation_type,$customer_role,$package,$request){
        return \App\Models\Transportation\SeatSegment::where(function ($q) use ($customer_role, $package) {
        ...
        ->whereIn('origin.code', ['EST','MTP','FBT','FCT']) // Set origin as CMY
    ...
    protected function departure_schedule($transportation_type,$arrival_schedules,$customer_role,$package,$request){
        return    \App\Models\Transportation\SeatSegment::where(function ($q) use ($customer_role, $package) {
            // $q->whereHas('allowed_roles', function ($query) use ($customer_role) {
            //         $query->where('role_id', $customer_role->id);
            // });
