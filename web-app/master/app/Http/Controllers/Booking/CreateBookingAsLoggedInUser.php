...
use App\Http\Requests\Booking\CreateBookingAsLoggedInUserRequest;
...
use App\Models\Booking\GuestTag;
...
class CreateBookingAsLoggedInUser extends Controller
{
    /**
     ...
     */
    public function __invoke(CreateBookingAsLoggedInUserRequest $request)
    {
        //
        // return $request->all();
        // customerWillReceiveBookingConfirmation
        $isUserAgent = $request->user()->user_type == 'agent' ? true : false;

        // return current && (current.valueOf() < (moment().hour() >= 14 ? moment().add(1,'days') : moment()) || current.valueOf() < moment("2020-12-01"))
        ...
        $isSameDayBooking = $arrival_date->format('Y-m-d') == Carbon::now()->setTimezone('Asia/Manila')->format('Y-m-d') ? true : false;

        // if (now()->hour >= 22 && now()->minute == 45) {
        //     return false;

        //     if ($arrival_date->lt(now()->add(1, 'day'))) {
        //         // return "Date cannot be booked.";
        //         return false;
        //     }
        // }

        // if (now()->hour >= 22 && now()->minute == 45) {
        //     return $arrival_date->lt(now()->add(1, ‘day’);
        // }
        ...
        $customer = Customer::firstOrCreate(
            ['email' => $request->user()->user_type == 'agent' ?  $request->email : $request->user()->email],
            [
                'object_id' => $request->user()->user_type == 'agent' ? null : $request->user()->object_id,
        ...
        $daily_limit = Setting::where('code', 'SALES_DAILY_LIMIT')->first()->value ?? 1400;
        ...
        // $check_daily_limit_per_day = DailyGuestLimit::whereIn('date', count($array_period_dates) > 0 ? $array_period_dates : [$arrival_date->format('Y-m-d')])->where('category', 'Sales')->get();
        $check_daily_limit_per_day = DailyGuestLimit::whereIn('date', [$arrival_date->format('Y-m-d')])->where('category', 'Sales')->get();
        ...
        $total_pax_no_infant = $adult_count + $kid_count;
        ...
        // Get agent details
        if ($isUserAgent) {
            $agent = \App\User::where('id', $request->user()->id)
                ->parentTeam()
                ->first()
                ->toArray();
        ...
        // Land Allocation Checking
        if ($request->mode_of_transportation != 'camaya_transportation' || $request->mode_of_transportation != 'camaya_transportation_ferry') {
            $land_allocations = LandAllocation::whereIn('date', $booking_dates)
                ->where('owner_id', $sales_director_id)
                ->where('entity', 'RE')
                ->where('status', 'approved')
                ->where(function ($q) use ($request) {
                    $q->whereRaw('land_allocations.id in (select land_allocation_id from land_allocation_users where user_id = ?)', [$request->user()->id]);
                    $q->orWhereRaw('json_contains(allowed_roles, \'["' . $request->user()->roles[0]['name'] . '"]\')');
                })
                ->get();
            ...
            foreach ($land_allocations as $land_allocation) {
                $available = ($land_allocation['allocation'] - $land_allocation['used']) - $total_pax_no_infant;
                ...
                LandAllocation::where('id', $land_allocation['id'])->increment('used', $total_pax_no_infant);
            }
        }
        ...
        if (!empty($request->selectedPackages) && count($request->selectedPackages) > 0) {


            if (!empty($request->selectedPackages[0]['arrival_schedules']) && count($request->selectedPackages[0]['arrival_schedules']) > 0) {

                if ($request->selectedPackages[0]['arrival_schedules'][0]['transportation']['type'] == "ferry") {
                    $modeOfTransportation = isset($request->guest_vehicles) ? 'own_vehicle' :  "camaya_transportation_ferry";
                } elseif ($request->selectedPackages[0]['arrival_schedules'][0]['transportation']['type'] == "bus") {
                    $modeOfTransportation = isset($request->guest_vehicles) ? 'own_vehicle' :  "camaya_transportation_bus";
                }
            } else {
                $modeOfTransportation = isset($request->guest_vehicles) ? 'own_vehicle' : $request->mode_of_transportation;
            }
        } elseif (!empty($request->selectedProducts) && count($request->selectedProducts)) {
            $modeOfTransportation = "camaya_transportation_ferry";
        } else {
            $modeOfTransportation = isset($request->guest_vehicles) ? 'own_vehicle' : $request->mode_of_transportation;
        }

        $newBooking = Booking::create([
            'user_id' => $request->user()->user_type == 'agent' ? null : $request->user()->id,
            ...
            'eta' => $request->eta,
            'portal' => $request->user()->user_type == 'agent' ? 'agent_portal' : 'website',
            'mode_of_payment' => $request->modeOfPayment,
            'auto_cancel_at' => Carbon::parse(strtotime($request->auto_cancel_at))->addHours(48)->setTimezone('Asia/Manila'),
            'created_by' => $request->user()->id,

            //Sales Agent ang Director Ids
            'agent_id' => $agent_id,
            'sales_director_id' => $sales_director_id,
        ]);
        ...
        foreach ($allGuests as $guest) {
            ...
            $guests_to_save[] = new Guest([
                'related_id' => $guest['related_id'] ?? null,
            ...
            // Create passes for Ala-carte room bookings
            if ($request->selectedRoomTypes && (!$request->selectedProducts && !$request->selectedPackages)) {
                Pass::createPassesByType(['Commercial Gate Entry Pass', 'Commercial Gate Exit Pass', 'Main Gate Access', 'AF Parking Entry', 'AF Parking Exit'], $booking_reference_number, $guest_reference_number, $arrival_date, $departure_date);
            }
        ...
        foreach ($newBooking->guests as $guestKey => $guest) {

            $_guest = $allGuests[$guestKey];
            ...
            foreach ($perGuestProducts->all() as $item) {
                ...
                if ($guest['type'] == 'kid') {
                    if ($isSameDayBooking) {
                        ...
                        $price = isset($item['kid_price']) ? $item['kid_price'] : $item['price'];
                    }
                }
                ...
                if (isset($_guest['is_foc']) && $_guest['is_foc'] == true) {
                ...
                if ($item['code'] != 'TRIPPING_FOC') {
                    ...
                    $per_guest_inclusions_inclusion = $newBooking->inclusions()->create([
                        ...
                        'selling_price' => $guest['age'] <= $infant_max_age ? 0 : $item['price'],
                        'discount' => null,
                        'created_by' => $request->user()->id,
                    ]);
                    ...
                    if (isset($_guest['is_foc']) && $_guest['is_foc'] == true) {
                        $per_guest_inclusions_foc = $newBooking->inclusions()->create([
                            'invoice_id' => $newBooking->invoices[0]->id,
                            'guest_id' => $guest['id'],
                            'guest_reference_number' => $guest['reference_number'],
                            'item' => $item['name'],
                        ...
                        foreach ($item['productPass'] as $product_pass) {
                            ...
                            if ($main_gate_pass->id != $product_pass['stub_id']) {
                                Pass::createProductPasses($product_pass['stub_id'], $booking_reference_number, $guest['reference_number'], $arrival_date, $departure_date, $per_guest_inclusions_foc->id);
                            }
                        }
                    }
                }
                ...
                //         // $invoice_total_cost = $invoice_total_cost + $price;
                //         $invoice_total_cost = $invoice_total_cost + 0;

                //         $per_guest_inclusions_to_save[] = new Inclusion([
                //             'invoice_id' => $newBooking->invoices[0]->id,
                //             'guest_id' => $guest['id'],
                //             'guest_reference_number' => $guest['reference_number'],
                //             'item' => $item['name'],
                ...
                //         GuestTag::create([
                //             'guest_reference_number' => $guest['reference_number'],
                //             'name' => 'SDMB - Sales Director Marketing Budget'
                //         ]);
                ...
                //             Pass::createProductPasses($product_pass['stub_id'], $booking_reference_number, $guest['reference_number'], $arrival_date, $departure_date);
            ...
            foreach ($perGuestPackages->all() as $item) {
                ...
                if ($nights >= 1) {
                    for ($i = 0; $i < $nights; $i++) {
                        ...
                        $price = $per_guest_package_selling_price;

                        if ($isSameDayBooking) {
                            $price = $item['walkin_price'] ? $item['walkin_price'] : $per_guest_package_selling_price;
                        }

                        // Update price per infant
                        $price = $guest['age'] <= $infant_max_age ? 0 : $price;
                        if (isset($_guest['is_foc']) && $_guest['is_foc'] == true) {
                            $price = 0;
                        }
                ...
                if ($selectedRoomTypes && $guest['type'] != 'infant') {
            ...
            if ($request->firstTrip) {
                ...
                $invoice_total_cost = $invoice_total_cost + ((isset($_guest['is_foc']) && $_guest['is_foc'] == true) ? 0 : $ticket_selling_price);
                ...
                $per_guest_inclusions_ticket_1 = $newBooking->inclusions()->create([
                    ...
                    'price' => (isset($_guest['is_foc']) && $_guest['is_foc'] == true) ? 0 : $ticket_selling_price,
                    'walkin_price' => 0,
                    'selling_price' => 0,
                    'discount' => null,
                    'created_by' => $request->user()->id,
        ...
        if ($isSameDayBooking) {
            $tags_to_save[] = new BookingTag([
                'name' => 'Walk-in - Sales Client',
                'created_by' => 0
            ]);
        }

        if ($focCount == $total_pax_no_infant) {
            $tags_to_save[] = new BookingTag([
                'name' => 'SDMB - Sales Director Marketing Budget',
                'created_by' => 0
            ]);
        ...
        if ($request->selectedRoomTypes) {
            ...
            foreach ($request->selectedRoomTypes as $room_types_to_reserve) {
                ...
                $room_capacity = $room_capacity + $room_type['capacity'];
            ...
            if ($extra_pax > 0) {
                ...
                if ($extraPaxProduct) {
                    ...
                    $invoice_total_cost = $invoice_total_cost + ($extraPaxProduct['price'] * $extra_pax);
                }
            }
        ...
        if (
            $booking->mode_of_transportation == 'camaya_transportation' || $booking->mode_of_transportation == 'camaya_transportation_ferry' || $booking->mode_of_transportation == "camaya_transportation_bus"
        ...
        $mailToSend = new NewBooking($booking, $camaya_transportations, $guest_id);


        /**
         * If user type is agent and the invoice cost is zero, auto-confirm the booking
         */
        if (
            ...
            /**
             * Confirm booking that's not van rental mode of transportation
             */
            if ($request->mode_of_transportation != 'van_rental') {
                ...
                $mailToSend = new BookingConfirmation($booking, $camaya_transportations, $booking->reference_number);
            }
        }

        if ($request->customerWillReceiveBookingConfirmation == false) {
            // $emailRecepient = $booking->bookedBy->email;
            Mail::to($booking->bookedBy->email)
